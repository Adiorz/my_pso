...
////    	std::cout << "freq_hist: " << psos->at(p)->get_gbest_from_history().at(1)/2/M_PI << std::endl;
...
////				psos->at(i)->get_gbest_from_history().at(0),
////				psos->at(i)->get_gbest_from_history().at(1),
////				psos->at(i)->get_gbest_from_history().at(2),
////				psos->at(i)->get_gbest_from_history().at(3),
//				time_real,
//				numofsamples,
//				A_approx[i]);
...
//	DataStream channel0("data/bp2_mod_90_01.lvm", 1, 5000);
	...
	std::vector<float> freq;
	get_frequencies(freq, numofsamples, fs);
//	std::cout << freq[454] << std::endl;
//	std::cout << numofsamples << std::endl;
//	std::cout << fs << std::endl;
//	std::cout << freq[454*numofsamples/fs] << std::endl;
	std::vector<float> A;
	std::vector<float> P;
//	std::vector<double> *used_data = &channel0_real;
	std::vector<double> *used_data = &data_derivative;
	...
	fft(*used_data, A, P);
	std::vector<float> A_gauss(gaussian_filter(A, 3));
	size_t f = psos->at(0)->getgbest().at(1)/2/M_PI*numofsamples/fs;
	std::cout << "used freq: " << psos->at(0)->getgbest().at(1)/2/M_PI << std::endl;
	...
	third.run();
	f = psos->at(1)->getgbest().at(1)/2/M_PI*numofsamples/fs;
	std::cout << "used freq: " << psos->at(1)->getgbest().at(1)/2/M_PI << std::endl;
	findMinimas(A_gauss, 0, f, idxL);
	findMinimas(A_gauss, 0, f, idxR);
	transform(idxR.begin(), idxR.end(), idxR.begin(),
	          bind2nd(std::plus<size_t>(), f));
